  ...
  sidebarLayout(
    ...
    shiny::column(4, offset = 4,
                  sliderInput(inputId = "year", 
                              label = sprintf("Center of Gaussian averaging", window.length),
                              min = min(anim.data$Year), max = max(anim.data$Year), step = 1/interp.resolution, 
                              #format = "####", 
                              ticks = F, round = T,
                              sep = "", pre = NULL, post = NULL,
                              value = min(anim.data$Year), width = '1000%',
